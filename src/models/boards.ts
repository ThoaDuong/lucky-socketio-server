...
export const boards_room: BoardRoom[] = [];

export const initBoardRoom = (username: string, room: string) => {
    let boardId = 0;
    for(let i=0; i<boards.length; i++){
        //check if boardId not exist in boards_room
        const index = boards_room.findIndex(b => 
            b.boardId === boards[i].id &&
            b.room === room);
        if(index === -1){
...
function getRandomInt (min: number, max: number, listNumbersCheck: number[]) {
    let randomTemp = 0;
    do{
        randomTemp = Math.floor(Math.random() * (max - min + 1)) + min;
    }while(listNumbersCheck?.indexOf(randomTemp) !== -1);
    return randomTemp;
}

export const autoGenerateRandomBoard = () => {
    // init all random number from 1 to 90
    let listNumbers = new Array<number>();
    
    // init all index list
    let listBoardIndexColumn = new Array<(number)[]>([], [], [], [], [], [], [], [], []);
    ...
    do {
        ...
        for(let i=0; i<=8; i++){
            ...
            if(i*10 <= ranNum && ranNum < i*10+10){
                const index: number = getRandomInt(0, 8, listBoardIndexColumn[i]);

                // condition: max number in columns is 5
                if(listBoardIndexColumn[i].length < 6){
                    let boardRow: (number|null)[] = board[index];
